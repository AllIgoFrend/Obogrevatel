...
{
    return i2c_send(dev, CMD_REGISTR, &cmd, 1);
}
...
{
    if (x >= dev->width || y >= (dev->height >> 3U))
        return -EINVAL;
    int err = 0;
...
int ssd1306_load_frame_buffer(const ssd1306_t *dev, uint8_t buf[])
{
    uint16_t i;
    uint8_t tab[16] = { 0 };
    size_t len = dev->width * (dev->height >> 3U);
    if (dev->screen == SSD1306_SCREEN) {
        ...
        ssd1306_set_page_addr(dev, 0, (dev->height >> 3U) - 1);
    }
 ...
 */
int ssd1306_load_xbm(const ssd1306_t *dev, uint8_t *xbm, uint8_t *fb, uint8_t x, uint8_t y, uint8_t h, uint8_t w)
{
    if((x+w) > dev->width || (y+h) > dev->height){
        return 0U;
    }
    //uint16_t index = 0U;
    //uint8_t bit = 0U;
    ...
    int err = 0U;
    uint8_t line = 0U;
    //index = x + (y >> 3U) * dev->width;
    for (row = 0U; row < h; row++) {
        uint16_t xbm_offset = row * ((w + 7U)>>3U);
        for (column = 0U; column < w; column++) {
            if ((column & 7U) == 0U) {
                line = xbm[xbm_offset + (column >> 3U)]; // line data
                //printf("  line %d   offset %i   column %i row %d\n",line ,xbm_offset, column, row );
            }
                if (line & 0x80) {
                    err = ssd1306_draw_pixel(dev, fb, x + column, y + row, OLED_COLOR_WHITE);     
                } else {
                    err = ssd1306_draw_pixel(dev, fb, x + column, y + row, OLED_COLOR_BLACK);     
                }
            if (err)
                return -ERANGE;
            line = line << 1; 
...
int ssd1306_fill_rectangle(const ssd1306_t *dev, uint8_t *fb, int8_t x, int8_t y, uint8_t w, uint8_t h, ssd1306_color_t color)
{
    ...
    */
    ...
    for (u8_t j = 0U; j < h; ++j) {                                                                          
        for (u8_t i = 0U; i < w; ++i) {                                      
            err = ssd1306_draw_pixel(dev, fb, x + i, y + j, color);     
            if (err)                                                         
                return -ERANGE;  
...
int ssd1306_draw_char(const ssd1306_t *dev, uint8_t *fb, const font_info_t *font, uint8_t x, uint8_t y, uint8_t length, char c, ssd1306_color_t foreground,
        ssd1306_color_t background)
{
    uint8_t i, j, l, offset_line = 0U;
    static const uint8_t *bitmap;
    uint8_t line = 0U;
    uint8_t line_be = 0U;
    uint8_t width = 0U;
    int err = 0U;
    uint16_t d = font_get_char(font, c);
    /*
...
//    bitmap = font->bitmap + d->offset;
    bitmap = font->bitmap + d;
    width = bitmap[0];
    bitmap = bitmap + 1U;
    //printf("  d %i   width %d\n",d, width );
    for (j = 0U; j < font->height; ++j) {                                    //подтиратель
        offset_line = (j*font->wids);                                        //подтиратель
        for (i = 0U; i < length; ++i) {                                      //подтиратель
            err = ssd1306_draw_pixel(dev, fb, x + i, y + j, background);     //подтиратель
            if (err)                                                         //подтиратель
                return -ERANGE;                                              //подтиратель
        }                                                                    //подтиратель
        //printf("  j %d   len %d\n",j , length );
        for (i = 0U; i < width; ++i) {
            if ((i & 7U) == 0U) {
                line = bitmap[offset_line + (i >> 3U)]; // line data
                if(!font->little_end){
                    line_be = line;
                    for(l = 0U; l < 8U; ++l){
                        line_be = line_be << 1U;
        ...
        //printf("  line %d   offset %d   char %d width %d\n",j ,offset_line, offset_line + (i >> 3U), width );
    }
    return width;
}

int ssd1306_draw_string(const ssd1306_t *dev, uint8_t *fb, const font_info_t *font, uint8_t x, uint8_t y, uint8_t length, const char *str,
        ssd1306_color_t foreground, ssd1306_color_t background)
{
    ...
    while (*str) {
        err = ssd1306_draw_char(dev, fb, font, x, y, length, *str, foreground, background);
        if (err < 0){
            //printf("  lenght %d   err %i   x %d str %s\n",length ,err, x, str );
            return err;
        }
        x += err;
        ++str;
